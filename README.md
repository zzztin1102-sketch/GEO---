...
> 基于大模型的 AI 内容合规审核平台，专为 **GEO（Generative Engine Optimization，生成式引擎优化）** 场景设计，支持多行业动态规则、LLM 可引用性分析、品牌实体一致性校验。
...
**GEO 生文审核系统** 不是一个简单的"合规检查工具"，而是一个 **智能审核 Agent**：

```
用户提交内容 → TaskPlanner 分析任务类型 → 动态选择审核策略
    → 规则引擎（快速过滤）+ LLM 语义审核（深度分析）
    → GEO 特色审核（可引用性 + 品牌一致性）
    → 结构化审核报告（问题列表 + 修改建议 + 修改清单）
```
