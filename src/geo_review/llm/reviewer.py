...
class ReviewResultMerger:
    """合并规则引擎和 LLM 审核结果."""

    @staticmethod
    def merge(rule_issues: List[Issue], llm_issues: List[Issue]) -> List[Issue]:
        """合并两类问题，按严重程度和类型排序，去重.

        ✅ 优化：去重逻辑增强
        - 使用 snippet[:60] 替代 snippet[:30]（减少误去重）
        - 新增模糊匹配：如果两个 snippet 有 80% 以上的字符重合，视为重复
        """
        seen = set()
        merged = []

        for issue in rule_issues + llm_issues:
            # 使用更长的 snippet 前缀作为去重 key
            key = (issue.type.value, issue.evidence.snippet[:60])
            if key in seen:
                continue
            # ✅ 新增：模糊匹配去重
            is_dup = False
            for existing in merged:
                if existing.type == issue.type:
                    s1 = issue.evidence.snippet[:80]
                    s2 = existing.evidence.snippet[:80]
                    # 简单的字符集重叠率检查
                    if s1 and s2:
        ...
        merged.sort(key=lambda x: (severity_order.get(x.severity.value, 9), x.type.value))
